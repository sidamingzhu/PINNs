...
data = scipy.io.loadmat('W:/documents/dev/python/PINNs/PINNs/vallina_pinns/Burgers/burgers_shock.mat')
...
idx_Xf = np.random.choice(xt.shape[0], 4000, replace=False)#随机选2000个点
...
X,T,x_bc,t_bc,u_bc,x_init,t_init,u_init,x_collocation_points,t_collocation_points=map(lambda x: tf.convert_to_tensor(x,dtype=tf.float64),[X,T,x_bc,t_bc,u_bc,x_init,t_init,u_init,x_collocation_points,t_collocation_points])
...
@tf.function
def train_model(x_bc,t_bc,u_bc,x_init,t_init,u_init,x_collocation,t_collocation):
    with tf.GradientTape() as tape:
        total_loss=bi_loss_fun(x_bc,t_bc,u_bc)+bi_loss_fun(x_init,t_init,u_init)+pde_loss_fun(x_collocation,t_collocation)
...
for epoch in range(2500):
    total_loss=train_model(x_bc,t_bc,u_bc,x_init,t_init,u_init,x_collocation_points,t_collocation_points)
    if epoch%10 ==0:
